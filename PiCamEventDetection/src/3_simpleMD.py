...
for f in camera.capture_continuous(rawCapture, format="bgr", use_video_port=True):
    ...
    frameDelta = cv2.absdiff(gray, cv2.convertScaleAbs(avg)) # delta = |previous_frame - current_frame|
    ...
    numWhite = cv2.countNonZero(thresh)
    print("Number of White Pixels: {}".format(numWhite))
    if numWhite > WHT_CNT :
        motionDetected = 1
